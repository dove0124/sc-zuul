...
public class AuthFilter extends ZuulFilter {
    ...
    public boolean shouldFilter() {
        ...
        //		System.out.println(rc.getRequest().getRequestURI());
        //		System.out.println(rc.getRequest().getRequestURL());
        if (rc.getRequest()
              .getRequestURI()
              .endsWith("/v2/api-docs")) {
            return false;
        }
        return true;
    }
    ...
    public static void authUser(RequestContext ctx) {
        ...
        if (StringUtils.isEmpty(userId)) {
            try {
                BaseException BaseException = new BaseException(CommonError.AUTH_EMPTY_ERROR.getCode(),
                                                                CommonError.AUTH_EMPTY_ERROR.getCodeEn(),
                                                                CommonError.AUTH_EMPTY_ERROR.getMessage(),
                                                                1L);
                BaseExceptionBody errorBody = new BaseExceptionBody(BaseException);
                ctx.setSendZuulResponse(false);
                ctx.setResponseStatusCode(401);
                ctx.setResponseBody(JSONObject.toJSON(errorBody)
                                              .toString());
            } catch (Exception e) {
